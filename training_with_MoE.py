...
init_from = 'resume'  # options: 'scratch', 'resume'
wandb_log = False
wandb_project = 'owt'
wandb_run_name = 'FIRA'
...
batch_size = 16
block_size = 512  # Sequence length/context length. this means how long it can remember
# in the past.
n_layer = 12
n_head = 12
n_embd = 768
...
dropout = 0.1
num_experts = 4
""" maximum learning rate. thats because we use cosine decay and warmup. and also its good practice to use maximum learning rate to capture as much data as possible in pretraining stage"""
learning_rate = 6e-4  
max_iters = 100
weight_decay = 1e-1
beta1 = 0.9
...
grad_clip = 1.0
decay_lr = True
warmup_iters = 70  # Reduced warmup to allow decay
lr_decay_iters = 20
min_lr = 5e-5
backend = 'nccl'
device = 'cuda'
...
"""
    you can assume why i change or strip the vocab size. its because i am using custom tokenizer and it has different vocabulary size.
    and why 50304? because 50304/16(the bloack size) = 3144(we count this value as number of embedding layer). so its divisible by 16.
    and also its good practice to use vocabulary size that is divisible by block size. and it makes it much more efficent.

"""
my_vocab_size = 50304 


# Build a config dictionary for logging.its not that important. we can use simple .json file to store the configaration.
...
""" Function to compute total parameters in the model
    This function calculates the total number of parameters in the FIRA model,"""
...
def compute_total_parameters(n_layer, n_head, n_embd, d_ff, num_experts):
    ...
    lm_head_params = n_embd * my_vocab_size
...
""" Function to convert pipeline state_dict to model state_dict
    This function maps the shapes of parameters in the pipeline state_dict to the model state_dict,
    ensuring that the shapes match and parameters are correctly aligned.
...
""" Initialize model and optimizer based on init_from option
    This section initializes the model and optimizer based on the specified init_from option.
"""
...
""" Define the FIRA model Embedding class
    This class implements the FIRA model architecture, including the embedding layer,
    transformer layers, and the final linear layer for language modeling.
"""
...
"""Helper function for hellaswag. i am using hellswag for evaluation. So thats why i am using it it can be used for other tasks too.
    its most likely was provided by the HellaSwag team in github official repository.
"""
...
""" Set up training parameters and gradient accumulation
    This section sets up the total batch tokens, gradient accumulation steps, and checks for divisibility.
"""
...
""" Function to generate text using the trained model
    This function generates text based on a given prompt using the trained model.
"""
...
""" Function to estimate loss on training and validation sets. its more like a utility function.
    This function evaluates the model on both training and validation sets to estimate the loss.
"""
...
"""training loop
    This is the main training loop that iterates through the dataset, performs forward and backward passes,
"""
...
""" Plot training progress
    This section plots the training and validation loss over iterations.
"""
